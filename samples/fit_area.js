...
if (!deserialize($projects["Area"]))
{
    ...
    new Fitting({ref: area_light});

    Variable.get("x", {min: 0, max: 4, res: 16});
    Variable.get("z", {min: 0, max: 4, res: 32});
    Variable.get("l", "0.6 * t");
    Variable.get("s", "0.4 * l");
    Variable.get("d", "-0.3 * s");
