...
function add_function(code, display)
{
    ...
    aceEditor.session.on('change', function(delta) {
        ...
        refresher = setTimeout(function() {
            ...
            let new_func = eval("(" + aceEditor.getValue() + ")");
...
function ensure_sliders()
{
    ...
    for (let i = 0; i < num_sliders; i++)
    {
        ...
        html += `</select>
        <label for="slider-${i}" class="form-label">${$settings.sliders[i].value}</label>
        <input type="range" class="form-range" id="slider-${i}" value="${$settings.sliders[i].value}" min="0" max="1" step="0.001" oninput="on_slider_change(${i})">
        </div>`;
    }
...
{
    let sliderDiv = document.querySelector('#sliders').children[index + 1]; // skip <br>
    let value = sliderDiv.children[2].value;
    sliderDiv.children[1].innerText = value;
    $settings.sliders[index].value = parseFloat(value);
    dirty_functions();
}
...
function sample_lut(name, x, y, channel)
{
    ...
    let x_lerp = (x - x_low);
    let low = lerp(load_lut(x_low, y_low), x_lerp, load_lut(x_low+1, y_low));
    let high = lerp(load_lut(x_low, y_low+1), x_lerp, load_lut(x_low+1, y_low+1));
